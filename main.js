...
function applyCharacterImageFocus(img, character) {
  const x = Number.isFinite(Number(character?.imagePosX)) ? Number(character.imagePosX) : 50;
  const y = Number.isFinite(Number(character?.imagePosY)) ? Number(character.imagePosY) : 18;
...
class CharacterModal extends Modal {
  ...
  onOpen() {
    ...
    let imagePosX = Number.isFinite(Number(this.character?.imagePosX)) ? Number(this.character.imagePosX) : 50;
    let imagePosY = Number.isFinite(Number(this.character?.imagePosY)) ? Number(this.character.imagePosY) : 18;
    ...
    const imageSetting = new Setting(this.contentEl)
      .setName("密探图片")
      .setDesc("缩略图固定 3:4，并放大 1.4 倍；下方可手动移动取景位置。");

    const previewWrap = this.contentEl.createDiv({ cls: "character-image-editor" });
    const previewFrame = previewWrap.createDiv({ cls: "character-image-preview-frame" });
    const previewImg = previewFrame.createEl("img", { cls: "character-image-preview" });
    const previewEmpty = previewFrame.createDiv({ cls: "character-image-preview-empty", text: "暂无图片" });

    const refreshPreview = () => {
      const src = imageSrc(this.app, image);
      if (!src) {
      ...
      previewImg.style.objectPosition = `${imagePosX}% ${imagePosY}%`;
      previewImg.style.transformOrigin = `${imagePosX}% ${imagePosY}%`;
    };
    ...
    const makeSlider = (label, desc, initial, onChange) => {
      const setting = new Setting(this.contentEl).setName(label).setDesc(desc);
      const wrap = setting.controlEl.createDiv({ cls: "image-position-control" });
      ...
      const valueEl = wrap.createSpan({ cls: "image-position-value", text: String(initial) });
      range.oninput = () => {
    ...
    makeSlider("左右移动", "0 最靠左，100 最靠右。", imagePosX, v => imagePosX = v);
    makeSlider("上下移动", "0 最靠上，100 最靠下。人物脸部偏低时把数值调小。", imagePosY, v => imagePosY = v);
  ...
  render() {
    ...
    for (const c of this.plugin.data.characters) {
      ...
      if (src) {
        ...
        img.src = src;
        applyCharacterImageFocus(img, c);
      } else {
...
class PoolModal extends Modal {
  ...
  onOpen() {
    ...
    let pityProgress = Math.max(0, Math.min(39, Number(this.pool?.pityProgress) || 0));
    ...
    new Setting(this.contentEl)
      .setName("当前已垫招募次数")
      .setDesc("上一次出绝密后，又招募了多少次但尚未出绝密。范围 0–39。比如第38次出绝密后又招募2次，这里填2。")
      .addText(t => {
        t.inputEl.type = "number";
        t.inputEl.min = "0";
        t.inputEl.max = "39";
        t.inputEl.step = "1";
        t.setValue(String(pityProgress)).onChange(v => {
          pityProgress = Math.max(0, Math.min(39, parseInt(v) || 0));
        });
      });
...
class PoolManageModal extends Modal {
  constructor(app, plugin, pool, onRefresh) {
    super(app);
    this.plugin = plugin;
    this.pool = pool;
  ...
  onOpen() {
    this.contentEl.empty();
    this.contentEl.createEl("h2", { text: this.pool.name });

    const progress = Math.max(0, Math.min(39, Number(this.pool.pityProgress) || 0));
    this.contentEl.createDiv({
      cls: "pool-manage-pity",
      text: `当前已垫 ${progress} 次 · 距离40次保底还有 ${40 - progress} 次`
    });

    new Setting(this.contentEl)
      .setName("新增招募记录")
      .setDesc("记录本次出绝密时的招募次数。保存后当前垫招募次数会归零。")
      .addButton(b => b.setButtonText("添加").setCta().onClick(() => {
        new RecordModal(this.app, this.plugin, this.pool, null, async r => {
          this.pool.records.push(r);
          this.pool.pityProgress = 0;
          await this.plugin.saveGachaData();
          this.onRefresh();
          this.close();
        }).open();
    ...
    new Setting(this.contentEl)
      .setName("编辑卡池")
      .setDesc("修改卡池资料、排序、UP密探，以及当前已垫招募次数。")
      .addButton(b => b.setButtonText("编辑").onClick(() => {
        new PoolModal(this.app, this.plugin, this.pool, async p => {
          Object.assign(this.pool, p);
          await this.plugin.saveGachaData();
          this.onRefresh();
          this.close();
        }).open();
    ...
    new Setting(this.contentEl)
      .setName("删除卡池")
      .setDesc("会同时删除该卡池内的招募记录。")
      .addButton(b => b.setButtonText("删除").setWarning().onClick(() => {
        new ConfirmModal(this.app, `确定删除「${this.pool.name}」及其记录？`, async () => {
          this.plugin.data.pools = this.plugin.data.pools.filter(x => x.id !== this.pool.id);
          await this.plugin.saveGachaData();
          this.onRefresh();
          this.close();
        }).open();
...
class GachaView extends ItemView {
  ...
  render() {
    ...
    [["tracker","招募记录"],["dex","密探图鉴"]].forEach(([value, label]) => {
  ...
  renderTracker(root) {
    ...
    const recordedPulls = allRecords.reduce((s, r) => s + Number(r.pulls || 0), 0);
    const pendingPulls = data.pools.reduce((s, p) => s + Math.max(0, Math.min(39, Number(p.pityProgress) || 0)), 0);
    const pulls = recordedPulls + pendingPulls;
    ...
    [["总招募次数", pulls], ["绝密记录", ssr], ["平均每个绝密", ssr ? pulls / ssr : 0]].forEach(([label, val]) => {
  ...
  renderPool(parent, pool) {
    ...
    const mainRow = box.createDiv({ cls: "pool-main-row" });
    const hero = mainRow.createDiv({ cls: "pool-hero" });
    ...
    const pityProgress = Math.max(0, Math.min(39, Number(pool.pityProgress) || 0));
    const poolTotal = records.reduce((s,r)=>s+Number(r.pulls||0),0) + pityProgress;
    overlay.createDiv({
      cls: "pool-stats",
      text: `${records.length} 次绝密 · ${poolTotal} 次招募`
    });
    overlay.createDiv({
      cls: "pool-pity",
      text: `距离40次保底还有 ${40 - pityProgress} 次`
    });
    ...
    const editWrap = mainRow.createDiv({ cls: "pool-side-edit" });
    const editButton = editWrap.createEl("button", { text: "编辑", cls: "mod-cta" });
    editButton.onclick = (ev) => {
      ev.stopPropagation();
      new PoolManageModal(this.app, this.plugin, pool, () => this.render()).open();
    };
    ...
    if (this.openPools.has(pool.id)) {
      ...
      details.createDiv({ cls: "pool-details-head", text: "招募记录" });
      ...
      records.forEach((r, i) => {
        ...
        card.createDiv({ cls: "pull-meta", text: String(r.pulls) });
...
class GachaTrackerPlugin extends Plugin {
  ...
  migrate(d) {
    ...
    for (const c of out.characters) {
      ...
      if (!Number.isFinite(Number(c.imagePosX))) c.imagePosX = 50;
      if (!Number.isFinite(Number(c.imagePosY))) c.imagePosY = 18;
      delete c.note;
      delete c.notes;
      delete c.remark;
    }
    ...
    for (let poolIndex = 0; poolIndex < out.pools.length; poolIndex++) {
      ...
      p.id = p.id || uid("pool");
      p.type = p.type || "up";
      if (!Number.isFinite(Number(p.pityProgress))) p.pityProgress = 0;
      p.pityProgress = Math.max(0, Math.min(39, Math.floor(Number(p.pityProgress) || 0)));
